...
| Layer | Choice | Why |
|---|---|---|
| **Mobile framework** | React Native + Expo (managed workflow: CNG/prebuild + EAS dev builds) | Single codebase; no local native toolchain. Native capability retained via config plugins + local Expo Modules; `npx expo prebuild` runs in EAS cloud only |
...
│
├── modules/
│   └── pose-detector/             # Local Expo Module: Kotlin + Swift MediaPipe bridge
├── assets/
└── package.json

# android/ and ios/ are generated by `npx expo prebuild` (CNG) inside EAS cloud builds
# and are never committed. Day-to-day dev runs in Expo Go / a dev build via Metro.
```
...
**`src/ml/poseLandmarker.ts`** — TS interface; real work happens in the local Expo Module `modules/pose-detector` (Kotlin/Swift), which loads the MediaPipe `.task` model and processes frame buffers natively. Built only by EAS cloud builds — identical JS API to a bare-workflow bridge.
