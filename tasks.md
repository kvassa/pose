...
### 5.1 Init Expo bare workflow

**Goal:** `apps/mobile/` runs an empty Expo app on iOS and Android.

**Steps:**
1. `cd apps && npx create-expo-app@latest mobile --template bare-minimum`.
2. Configure to use the workspace TS config.
3. Add to pnpm workspace.

**Test:** `pnpm --filter mobile run ios` and `... run android` both launch the default screen.

**Out of scope:** No screens, no deps.
...
**Goal:** `react-native-vision-camera` v4+ installed and pods/gradle synced.

**Steps:**
1. Install + run `pod install`.
2. Configure permissions in Info.plist and AndroidManifest.

**Test:** App still boots on iOS and Android.
...
**Goal:** Kotlin module `PoseModule.kt` exposes `detect(frame): keypoints` to JS.

**Steps:**
1. In `android/app/src/main/java/.../pose/`, create `PoseModule.kt` and `PosePackage.kt`.
2. Add MediaPipe Tasks dependency in `build.gradle`.
3. Implement `detect(byteArray)` that runs the pose landmarker and returns 33 landmarks.
4. Register in `MainApplication`.

**Test:** From JS, `NativeModules.PoseModule.detect(<test image bytes>)` returns 33 landmarks for a known image.

**Out of scope:** No frame processor integration.

### 9.2 Add native MediaPipe module — iOS

**Goal:** Mirror 9.1 in Swift.

**Steps:**
1. Create `PoseModule.swift` and bridging header.
2. Add MediaPipe Tasks pod.
3. Mirror Android API.

**Test:** Same JS call works on iOS.
...
**Steps:**
1. Create `PoseFrameProcessorPlugin.kt`.
...
**Goal:** `@shopify/react-native-skia` installed and pods synced.

**Steps:** Install + pod install + rebuild.
